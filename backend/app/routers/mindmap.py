...
from ..services.ai_client import call_ai, get_current_ai_settings

router = APIRouter()
logger = logging.getLogger(__name__)


def _build_mindmap_prompt(subject: str, aspects: list[str]) -> str:
    aspects_str = ", ".join(aspects)
    return f"""You are a professional systems analyst. Build a comprehensive mind map analysis of: "{subject}"
across these dimensions: {aspects_str}.
...
async def generate_mindmap(body: MindMapRequest, db: Session = Depends(get_db)):
    ...
    prompt = _build_mindmap_prompt(body.subject, body.aspects)
